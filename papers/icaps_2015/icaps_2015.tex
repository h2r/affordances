...
prior experience in related simpler problems than can be more easily solved
with an uniformed planner. Due to the large complexity of
the space, we use the video game Minecraft as a robot simulator in
which an agent learns useful affordances from a set of simple training worlds
...
affordances outperform baselines on difficult problems, our approach
is able to learn affordances from experience on simple, tractable, 
training problems that yield even greater performance on the difficult problems
than manually provided affordances.
%affordances learned from experience on
%experience yield even greater improvements.  
...
Following STRIPs-like terminology (in which a domain defines set objects,
predicates, and actions, and a problem defines a goal condition and 
initial state for a domain),
we define a domain as an OO-MDP except for the reward function and terminal
states; and a problem of the domain as the same OO-MDP with
a reward function, terminal states, and an initial state. We are
...
defined by a reward function plus the set of terminal states.
Figure~\ref{fig:oomdp} shows part of the OO-MDP domain definition
for Minecraft that we used and the kinds of goals used in different
...
%states.  Analogously, a STRIPs domain becomes an MDP definition, except
%for the reward function and terminal states; and the STRIPs problem
%becomes a complete MDP with the same elements from the domain, a reward
%function, set of terminal states, and an initial state.


%STRIPs problem is a specific MDP with a known reward function, set
%of terminal states, and an initial state.  
