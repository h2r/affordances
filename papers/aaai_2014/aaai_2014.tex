...
However, many robotics problems are of such exceeding complexity that formalizing them as mentioned results in an immense state-action space. This large state-action space, in turn, restricts the classes of robotics problems that are computationally tractable.
  For example, when a robot is manipulating objects in an environment
an object can be placed anywhere in a large set of locations. The size of the state space increases exponentially with the number of objects, which bounds the placement problems that the robot is able to expediently solve.\enote{we need to be careful about state space vs state-action space...}
%~\cite{grounds05}.
...
function \enote{aren't they though -- like if we suddenly made lava good wouldn't that mess up our agentLookingAtLava affordance?} \gnote{Your example would affect the planner (and the affordances would make planning take longer) but they can be learned independent of the reward function.} or state space, and provide the agent with transferable
...
\mathcal{A}_{\Delta} = \left( \bigcup\limits_{\Delta_{active}} \Delta.getActions(s) \right) \subseteq \mathcal{A}
\end{align}
\begin{algorithm}
...
\end{equation}
\gnote{The previous sentence seem unnecessary given that we say we want $A_{\Delta} = A^o$ and have the equation.}

Since our affordance actions are defined as the union of the actions of all active affordances, we can combine equations \eqref{eq:afford_union} and \eqref{eq:opt}, where each affordance contributes a set $\mathcal{A}' \subseteq A_{\Delta}$: 
\begin{align}
...
%We complete the expansion of our probability with a prescription of the probability distribution from which each affordance retrieves its action set. We employ a Dirichelet-multinomial coupled with a Dirichelet. A Dirichelet-multinomial is a distribution over multinomial distributions.
