...
% -- Comment commands --
 \newcommand{\stnote}[1]{\textcolor{Blue}{\textbf{S:}}}
 \newcommand{\dnote}[1]{\textcolor{Green}{\textbf{D: }}}
 \newcommand{\enote}[1]{\textcolor{Red}{\textbf{E: }}}
 \newcommand{\gnote}[1]{\textcolor{Purple}{\textbf{G: }}}
 \newcommand{\jnote}[1]{\textcolor{Orange}{\textbf{J: }}}
...
the robot is able to expediently solve.  Moreover, the task is made further
difficult by the fact that, most of these locations are irrelevant: when making brownies,
...
state and goal to be solved in a planning problem, we can evaluate whether $\delta$ is relevant by checking if the predicate of $\delta$ is true in the current state and if the current goal is equivalent to the goal type of $\delta$. This relevance is defined with the indicator function $f$:
...
This distribution may be modeled in a number of ways making this approach quite flexible, though we have chosen to model it as a Naive-Bayes. 
...
Finally, we define the prior on the optimality of each action to be the fraction of the time each action was optimal during training. This distribution fully describes the model used by an affordance-aware planner. 
...
on a state by state basis, focusing the agent on relevant action possibilities of the environment and
...
listed in Figure \ref{fig:afford_kb_exp} \dnote{Figure coming soon}. Additionally, we ran our full
...
Trench 	& \texttt{RTDP}  	& 16183.5 ($\pm$4509) 		& -8.1   ($\pm$2) 		& 53.1s   ($\pm$22) \\
		& \texttt{LHRTDP} 	& 11758.4 ($\pm$2815) 	& {\bf -8.7}   ($\pm$1) 		& 57.9s   ($\pm$20) \\
...
for RTDP, learned affordance-aware RTDP (LARTDP) and expert affordance-aware RTDP (EARTDP) after planning in 20 different maps of each goal type (100 total). Because the planners were forced to terminate after only 1000 rollouts, they did not always converge to the optimal policy.
In every task, affordance-aware-RTDP outperformed or performed comparably to RTDP. As the results suggest,
LARTDP found a similar quality plan (-9.8 reward to -8.6 reward) in significantly fewer Bellman updates than RTDP in the gold smelting
task (2822 Bellman updates to 30995), as well as the plane traversal task (19090 Bellman updates to 52408, with a reward of -7.5 to -82.6).
Expert affordances outperformed both RTDP and LARTDP on the Trench task, finding a comparable plan in significantly fewer Bellman updates.
...
\begin{tabular}{ l l  | c c c c}
  Planner & Augmentation 			&	Bellman	&	Reward 	& CPU \\ \hline
RTDP &  \texttt{Vanilla}  			&	27439($\pm$2348)	&	-22.6($\pm$9)	&	107($\pm$33)		\\
 & \texttt{w/ Opt}  				&	26663($\pm$2298)	&	-17.4($\pm$4.0)	&	129($\pm$35)		\\
 &\texttt{w/ MA}  				&	31083($\pm$2468)	&	-21.7	($\pm$5.0)	&	336($\pm$28)		\\
 &\texttt{w/ MA+Opt}  			&	27143($\pm$2380)	&	-16.9($\pm$3.0)	&	323($\pm$38)		\\ \hline
 Affordance- &\texttt{Vanilla}  	& 	9935($\pm$1031)&	-12.4($\pm$1.0)	&	\bf{53}($\pm$5.0)		\\
Aware  &\texttt{w/ Opt}  				& 	\bf{9675}($\pm$953)	&	\bf{-11.5}($\pm$1.0)	&	93($\pm$10)		\\
  &\texttt{w/ MA}  				& 	9854($\pm$1034)	&	-11.7($\pm$1.0)		&	162($\pm$17)		\\
  &\texttt{w/ MA+Opt}  			& 	10622($\pm$1046)	&	-13.4($\pm$1.0)	&	237($\pm$29)		\\
\end{tabular}
