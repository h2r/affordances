...
pruning, allowing for state space independence.
...
know the subset of actions that were optimal each state for the given goal, in 
...
the bellman equation/value function above, too?}
...
action set for a given state ($s$), goal ($G$), and knowledge base ($K$):
...
location, or crafting a structure. 
...
When a precondition and goal type ($\langle p , g \rangle = \delta$) is paired with a current
state and goal to be solved in a planning problem, we can evaluate whether $\delta$ is relevant
to the current circumstance by checking if the predicate of $\delta$ is true in the current state
and if the current goal logically entails\dnote{I originally changed equality to entailment - do you all think this is the right move? This is what we'd like eventually, but currently we just check equivalence (in the code). It's easy enough to change here} the goal type of $\delta$. This relevance is defined with the indicator function $f$:
...
Evaluating the relevance of each $\delta_j$ gives rise to a set of binary features,
...
We have chosen to model it as a Naive-Bayes, assuming a uniform prior on the optimality of
each action, and assuming that the features are conditionally independent of one another. 
...
Next we assume that each feature is independent of the others, given whether the action is optimal:
