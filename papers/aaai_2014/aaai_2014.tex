...
Furthermore, we show that an agent can learn affordances through
...
the soy sauce and saute pan are not.  For a different task, such as
...
% \dnote{Should we mention
%   heuristic planners or action pruning work here too?}

% stefie10: I think it's okay not to mention them here.  We want to
% highlight the closest areas in the introduction; we can save the
% full thing for related work.

%% \dnote{Does this paragraph feel necessary? We talk
%%   about options/macroactions so it makes sense to gloss over our other
%%   related work.}
...
furnishes, either for good or ill".  We define an affordance,
$\Delta$, as the tuple $\langle p, g, \delta, \alpha, \beta \rangle$,
where:
\begin{itemize}
...
%\item $\alpha$ is a vector of counts of size $\mathcal{A}$.
%\item $\beta$ is a vector of counts of the integers from 1 to $|\mathcal{A}|$.
\end{itemize}
