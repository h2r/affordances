...
% I think it's fine to go straight to OO-MDPs.  This sentence was
% hanging at the end of the MDP paragraph and wasn't a good open for
% OO-MDPs.  ``A classic way to provide a factored representation of an
% MDP state is to represent each MDP state as a single feature
% vector.''  A vectorized MDP state can be equivalently understood as
% the set of all the object states, $s \in {\cal S} = \cup_{i = 1}^o
% \{o_i.state\}$, in an OO-MDP.  -- stefie10
...
%To perform action pruning, we model the distribution over the optimal
%OO-MDP action set. We define the optimal action set, $\mathcal{A}^*$,
%for a given state $s$ and goal $G$ as:
%% -- Equation: Optimal Action Set --
%\begin{equation}
%\mathcal{A}^* = \left\{ a \mid Q^*_G(s,a) = V^*_G(s) \right\}
...
%We induce a distribution over the optimal action set, in a particular
%state $s$, under a goal $G$, and given a knowledge base $K$.%
...
%The agent uses this distribution to estimate the optimal action set
%for each state-goal pair, based on the set of available affordances.
%To formalize a knowledge base of affordances, we represent $K$ as a
%set of paired state preconditions and goal types, which induce a
%distribution over the resulting optimal action set.  Formally, $K$
%consists of a set of paired preconditions and goal types, $\delta_1
%\ldots \delta_{|K|}$, and a parameter vector $\theta$.  Each
%$\delta_j$ is a precondition and goal type pair, $\langle p, g
%\rangle$.  The parameter vector, $\theta_{ij}$ represents the weight
%associated with action $i$, for $\delta_j$. Here, $p \in \mathcal{P}$
%is a {\it predicate} in predicate space, $\mathcal{P}$, and $g \in
%\mathcal{G}$ is a {\it goal type} in goal space, $\mathcal{G}$. We
%rewrite Equation~\ref{eq:master} replacing $K$ with its constituents:
%% -- Equation: replace K --
%\begin{equation}
%\Pr(\mathcal{A}^* \mid s, G, K) = \Pr(\mathcal{A}^* \mid s, G, \delta_1 \ldots \delta_{|K|}, \theta)
...
%\begin{equation}
%= \prod_{i=1}^{|\mathcal{A}|} \frac{\sum_{\Phi_j} \Pr(\phi_1, \ldots, \phi_{|K|}, \theta_i \mid a_i) \Pr(a_i)}{\Pr(\phi_1, \ldots, \phi_{|K|}, \theta_i)},
%\label{eq:posterior}
%\end{equation}
%where $\Phi_j$ is the set of all possible feature vectors that have feature vector $\phi_j$ fixed to its actual value in the input state.
...
%A specific affordance refers to the action distribution associated
%with a single feature being active, while marginalizing out the rest
%of the features:%
...
%To learn affordances, we estimate the parameter vector $\theta$ that
%maximizes the accumulated reward during training, given a set of
%training worlds, $W$:
%\begin{equation}
%\argmax_\theta \sum_{w \in W} \sum_{s \in w} \Pr(\mathcal{A}^* \mid s, w.G, K)
%\end{equation}%

%We rewrite using Equation~\ref{eq:posterior}:
%\begin{equation}
...
%Where $C(\phi_j \mid a_i)$ denotes the counts of the number of times
%feature $\phi_j$ was active in states where $a_i$ was optimal during
%training. To inform these counts, we randomly generate a set of
%training worlds, $W$, that are small enough to synthesize a policy
%using tabular methods (i.e. thousands of states). We generated $W$ so
%that each of the possible goal types was assigned to a subset of the
%generated worlds.
