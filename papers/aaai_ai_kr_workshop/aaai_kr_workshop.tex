...
optimally \gnote{Change to near-optimally?}. We demonstrate our approach in the Minecraft domain as a
...
%\subsection{Affordances}
%Affordances have been used throughout Artificial Intelligence and Robotics
%for a variety of applications. \citep{steedman02} performed work in formalizing affordances through the 
%Linear Dynamic Event Calculus (LDEC). This work was aimed at linking events and 
%objects in the context of language and prelinguistic cognitive apparatuses. ~\citep{koppula13a} 
...
Additionally, we tested our learning procedure and compared the performance of \gls{rtdp} solving the \gls{oomdp}
...
\gnote{Mention the sizes of $\mathcal{P}$ and $\mathcal{G}$ and give a couple of examples here.}
...
agent needs to evaluate in order to act optimally \gnote{Change to near-optimally?}. We demonstrated the effectiveness of the affordance model by comparing standard paradigm
