...
Likewise, when the agent is attempting to satisfy a different goal or is {\it not} looking at a block of gold, then the \texttt{destroy}
action is generally not optimal (i.e. destroying grass blocks is typically irrelevant to the smelting task). 
This information informs the distribution over the optimality of the \texttt{destroy} action given the set
...
To fix this, we could lower the threshold to allow for even more conservative action pruning at the cost
of additional exploration of the state-action space. In future work, we plan on investigating approaches
to dynamically adjusting the threshold based on planning feedback. 
%Additionally, RTDP often achieved a faster clock time because the affordance approaches evaluate the predicates associated with the affordances in each state.
%ToDo: put in a note about time!!!
...
\begin{figure}[b]
\centering
\includegraphics[width=0.7\linewidth]{figures/baxter.png}%
  \caption{Affordances enable a robot to efficiently infer helpful actions in
    very large state spaces, such as a kitchen.}
  \label{fig:baxter_results}
\end{figure}
