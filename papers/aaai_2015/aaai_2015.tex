...
\begin{align}
C(a_i) &= \sum_{w \in W} \sum_{s \in w} (a_i \in \pi(s)) \\
C(\bar{a_i}) &= \sum_{w \in W} \sum_{s \in w} (a_i \not \in \pi(s) ) \\
C(\phi_j, a_i) &= \sum_{w \in W} \sum_{s \in w} (a_i  \in \pi(s) \wedge \phi_j == 1)
\end{align}

\noindent We determined optimality using the inferred policy, $\pi$.

%Todo: Make this sound better
During the learning phase, the agent learns which actions are useful under different conditions. More formally,
it learns the model parameter $\theta$ for the distribution
on the optimality of each action (i.e. Equation ~\ref{eq:feature_rep}), given the features $\phi_1, \ldots, \phi_n$.
...
Likewise, when the agent was {\it not} looking at a block of gold or was attempting to satisfy a different goal, then the \texttt{destroyBlock}
action was generally not optimal. This information informs the distribution over the optimality of the \texttt{destroyBlock} action given the set
of features, which is used at test time to suggest that the agent ought to destroy gold blocks (when trying to smelt gold), but not arbitrary other blocks.

%determine which actions are likely to be relevant in the present
%state, and ignore those that are unlikely to be optimal.
%Each feature $\phi_j$ effects the planner's belief about the optimality of
%each action -- thus, with 16 predicates and 4 goal types, the planner is capable of making these inferences
%in a variety of scenarios. To generalize further, we may easily add additional predicates and goal types
%to the learning process. The difficulty here is creating a means of generating state-spaces in which each provided goal type may be satisfied.
...
vanilla RTDP (e.g. the mining task). This is likely due to the fact that LRTDP
occasionally prunes away actions that are essential for solving a particular problem.
To fix this, we could lower the threshold to allow for even more conservative action pruning
at the cost of performance. In future work, we plan on investigating more principled approaches to dynamically
adjusting the threshold based on planning feedback.
...
\bottomrule
\end{tabular}
\caption{RTDP vs. Affordance-aware ERTDP for robotic kitchen tasks}
\label{table:baxter_results}
\end{table}

In table~\ref{table:baxter_results} we show the comparison between
standard RTDP and ERTDP for the three subgoals. Because the number of
states and actions is reduced significantly,
...
We provide a video \footnote{See at https://vimeo.com/106226282} 
showing how this affordance-based planner running
