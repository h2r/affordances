...
speed, reductions in state space exploration, and improvements in the quality of the synthesized policy during planning.
...
and $C(\phi_j, a_i)$ is the number of occurrences where $\phi_j=1$ and $a_i$ was optimal. In all cases,
optimality was determined according to $\pi$.
...
\begin{figure}
\centering
\includegraphics[scale=0.09]{figures/smelting.png}%
...
\begin{figure*}
\centering
\includegraphics[scale=0.06,width=\textwidth]{figures/average_results.png}%
...
temporally extended actions, and both. The results suggest that affordances may be combined with 
temporally extended actions to provide improvements in planning, since affordances can handle the 
extended action space.

Lastly, we deployed an affordance-aware planner on a robot in a collaborative cooking task with a massive 
state space. \dnote{More baxter details here}.
...
Since affordances are goal specific, they will plug easily into high-level subgoal planning. Automatic discovery of subgoals
would allow affordance-aware planners to take advantage of this connection, and would further reduce the size of the 
explored state-action space by improving the effectiveness of action pruning. 
