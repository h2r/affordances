...
of all the object states: $s \in {\cal S} = \cup_{i = 1}^o \{o_i.state\}$. \gnote{Is this last equation right? If $o_i.state$ is an attribute assignment for one object then wouldn't $s$ be one object's attribute assignment, and $\mathcal{S}$ be a state.}
