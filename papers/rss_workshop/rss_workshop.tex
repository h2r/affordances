...
specifies which actions an agent should consider in different kinds of states to achive a certain kind of goal.
Our apporach enables an agent to focus on
...
%knowledge provided to an agent
%operating in a Markov Decision
%Process (MDP). 
...
future methods that bring together affordance-aware planners with these approaches to affordances
...
sensor data about the Minecraft world.
...
%\gnote{Is this last equation right? If $o_i.state$ is an attribute assignment for one object then wouldn't $s$ be one object's attribute assignment, and $\mathcal{S}$ be a state.}
%JM - yes, it is correct. Note that s is being assigned a *union* of sets of a single objects, which gives
% you a set of objects for the state.
...
a drawn action subset from the OO-MDP action set $\mathcal{A}$ that is likely to contain the optimal action(s) for a given state,
...
\begin{align}
\text{Pr}(\mathcal{A}'_0 \cup \ldots \cup \mathcal{A}'_N \mid s, \Delta_0 \dots \Delta_N)
\end{align}
...
affordance to always suggest a specific set of actions - this type of expert affordances was given for all
experiments.
...
that value for $\Delta_i.\beta$. This captures how large or small optimal action sets are expected to be.
...
the OO-MDP with training data of 0 up to 1000 simple state spaces to demonstrate the effectiveness of added training data.
...
there is clearly still room to improve the learning process to learn near-expert level affordances.
\begin{table}
\centering
...
doors. Although the classic options framework is not generalizable to different state spaces,
...
The primary difference between this body of work and our affordance formalism is that affordances may be learned, increasing autonomy of the agent, while LTL formulas are far too complicated to learn effectively, placing dependence on an expert.
