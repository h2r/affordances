...
Given the set of predicates $\mathcal{P}$ and possible goals $\mathcal{G} \subset \mathcal{P}$, we first form a set of candidate affordances $\Delta$ with every combination of $\langle p, g \rangle$, for $p \in \mathcal{P}$ and $g \in \mathcal{G}$.
...
when each affordance was activated. $\alpha$ is set to this count. Then, we count the number
of unique actions used by each policy and set $\beta$ to this value.
