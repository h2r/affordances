...
$\mathcal{T}(s' |\ s,a)$, the transition probability of an agent
...
Let any planner (MDP solver, etc.) that is equipped with a knowledge base of Affordances be known as {\it affordance-aware}.
...
First, we modify our original formalism to account for the lack of expert knowledge. If we kept the same formalism as in the expert
case, then our learned affordances would often toss out actions. Since the
learned affordances are more prone to make mistakes, we can't prune actions in this extreme way,
...
For each affordance we sample from our distribution over $N$ to get a candidate action set size and then
take that many samples from our distribution over $\lambda$ to get a candidate action set $\mathcal{A}'$.
