...
simple scenes from a Minecraft world appear in Figure~\ref{fig:epicworld}.   Miencraft serves as a model for robotic tasks such as cooking assistance, assembling items in a factory, and object retrieval.  As in these tasks,  the agent operates in a very large state/action space in an uncertain environment. 
...
\stnote{I think we should say something about why OO-MDPs don't solve
  the problem, which will transition into the next section.  Why don't
  OO-MDPs solve minecraft?}
...
affordance aware methods significantly outperformed its unaugmented
counterpart in all of these experiments. These
results, while unsurprising, concretely demonstrate that a small set of task-general affordances prune away many useless actions across many different types of Minecraft tasks. 
...
\begin{algorithm}
  \caption{$\Delta_i.getActionsForState(s)$}
  \begin{algorithmic}[1]
...
\end{align}
\stnote{I'm not sure we need this equation...}
...
\stnote{I'm confused about this jump.  HOw does this connect to equation 3?}
\begin{align}
...
\stnote{Oh I see.  This equation should go first, after equation 4.  Then give equation 5 and 6.}
\begin{equation}
...
Then, we randomly generate a large number of state spaces which are small enough to be solved using tabular methods (typically on the order of several thousand), annotated with their
lifted goal description $g \in \mathcal{G}$. We solve the OO-MDP in each state space to find an optimal policy $\pi_j$. For each optimal policy,
