...
public class Result {
	...
	private double avgAccumulatedReward = 0.0;
	...
	private void computeAverages() {
		...
		for(int i = 0; i < this.numTrials; i++) {
			if(this.completedTrials.get(i) == 1.0) {
		...
		this.completedRate = totalFinishedTrials / this.numTrials;
	...
	private void computeDeviations() {
		...
		if(this.numCompleted <= 1) {
			...
			for (int i = 0; i < this.numTrials; i++) {
				if(this.completedTrials.get(i) == 1.0) {
					...
					sumOfAvgDiffRewardSqd += Math.pow((this.rewardTrials.get(i) - this.avgAccumulatedReward),2);
		...
		for (Double df : completedTrials) {
			sumOfAvgDiffCompeletedSqd = Math.pow((df - this.completedRate),2);
		}
