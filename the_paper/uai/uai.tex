...
knowledge provided to an agent operating in a Markov Decision Process
(MDP). We explain how affordances can be leveraged by a variety of
planning algorithms to prune the action set the agent uses dynamically
based on the agent's current goal.  We call any planning algorithm
that uses affordances to prune the action set an {\it
  affordance-aware} planning algorithm.  Affordances are not specific
to a particular reward function or state space, and thus, provide the
agent with transferable knowledge that is effective in a wide variety
of problems. Because affordances define the {\em kind} of goals for
which actions are useful, they enable high-level reasoning that can be
combined with approaches like subgoal planning for even greater
performance gains. Figure \ref{fig:epic_world} shows the planner
acting in a complex the effectiveness of affordance-aware subgoal
planning on a complicated planning task in the Minecraft domain - a
video is provided of the agent achieving this task.  We let other
standard planners try to solve this task for (TIME), but they all
failed to converge on a policy (while affordance-aware subgoal planner
found a near-optimal policy in less than 5 minutes). \dnote{First
  pass, will need to make stronger when we add figures}
...
\caption{Subgoal planner in \texttt{BRIDGEWORLD}\stnote{I think we should cut this figure... it's confusing and we're short on space.}}
\label{fig:bwsg}
\end{figure}
...
agent was given anywhere from 4-15 affordances, depending on the
task. \jmnote{How many affordances were defined? We might want to list
  them all unless it's really large.}\dnote{Could consider including a
  few example sets as an appendix/footnote? Without directioned
  affordances, we only had 6 (at most), now we have about 15.}
\stnote{I don't think listing them all is necessary, as long as we
  have examples in the paper.  How come we can't use the same set for
  all domains?  That's a major power, that we can have one consistent
  set of affordances for everything.} Our experiments consisted of a
...
\dnote{Insert 4 frame Picture of epic world here, maybe include link to video instead of comment about supplementary material}
