...
\dnote{New title proposal: Affordance-Aware Planning}
...
furnishes, either for good or ill.''
      ...
      \jmnote{I added a sentence here on the utility of 
      affordances having a goal condition, because I think one
      of the cool contributions is how affordances
      can be integrate and strongly benefit subgoal planning,
      which other action pruning/prior work cannot do. We might
      want to think about making this a stronger point than I have
      though.}\dnote{Sounds good to me!}
...
a real world agent (e.g. a robot).  For instance, robotic agents are
...
were modified to have stochastic outcomes. These stocahstic outcomes
...
the agent closer to the goal. Thus, when the agent is in state under which block
...
\dnote{After hearing George's concerns about using the term affordance, I think we need to include affordance background up here. The reader needs to be aware of how we are interpreting Gibson's definition, and how that translates to our formalism in the next section. I propose we split the background into two subsections, OO-MDPs/planning, and Affordances. After the RelatedWork.Affordances section is complete, I will fill this section in, too (though it is sort of odd to have Affordances in background and its own section}
...
Minecraft. As discussed above, Gibson proposed that agents perceive
the environment in terms of affordances - further, which affordances are
perceived is determined by the agent's current goal. We capture this intuition via an affordance $\Delta$, defined 
...
an object of a certain type (e.g. baking bread, building a pickaxe). \dnote{Could consider adding a note that
ties each piece more closely to affordance literature (i.e. showing that $p$ is equivalent to observation of
...
%  OO-MDP, but above isn't quite right. } \dnote{Does this version work? Do we need to be more specific?}
...
{\em dynamic programming}, {\em policy rollout}, and {\em subgoal} planning paradigms. \jmnote{algorithm 1 needs to be referenced and explained in english.}
...
defined using OO-MDP predicates, each subgoal may activate a different set of affordances, thereby even more greatly focusing the search. \jmnote{This last sentence I added is correct, right?}.
...
the affordances provided from our background knowledge of the domain. \jmnote{How many affordances were defined? We might want to list them all unless it's really large.} Our experiments consisted
...
executed in each iteration of each planning algorithm. Value Iteration was terminated when the maximum change in the value function was less than 0.01. RTDP terminated when the maximum change in the value function was less than 0.01 for ten consecutive policy rollouts. In subgoal planning, the high-level subgaol plan was solved using breadth-first search; which only took a small fraction of the time compared to the total low-level planning and therefore is not reported. \jmnote{Check this. Also, probably need to specify how the reward function was defined and the discount factor used.}

Our entire set of experiments were repeated twice, the first iteration using deterministic actions, and the second iteration using stochastic actions. 

\subsection{RESULTS}

\dnote{Results table is {\bf complete} for both deterministic and non-deterministic tasks, but we added some metrics. Now, for each task, for each planner, we get the number of bellman updates, the total accumulated reward in executing the resulting plan/following the returned policy, and a flag indicating if the plan got the agent to the goal or not. We want to chat with everyone about how to best to display these results (tables, charts, etc.)}
...
There are a few limitiations of the actions priors work that affordance-aware planning does not possess: (1) the action priors can only be used with planning/learning algorithms that work well with an $\epsilon$-greedy rollout policy; (2) the priors are only utilized for fraction $\epsilon$ of the time steps, which is typically quite small; and (3) as variance in tasks explored increases, the priors will become more uniform. In contrast, affordance-aware planning can be used in a wide range of planning algorithms, benefits from the pruned action set in every time step, and the affordance defined lifted goal-description and enables higher-level resoning such as subgoal planning. However, in the future, the action set each affordance defines could be learned using a similar approach.
