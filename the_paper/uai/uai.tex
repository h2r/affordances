...
\begin{tabular}{ l || c | c | c | c | c | c }
  ...
  \hline
  \texttt{4BRIDGE} 		&	51867 		& 2285 		&	2100 	& 25990  		& 2075 	& 2304  \\
  \texttt{6BRIDGE} 		&	234090 	 	& 5994 		& 	7770 	& 93840  		& 4105 	& 6213  \\
  \texttt{8BRIDGE} 		&	679320 	 	& 20682 		&	18400 	& 226320		& 5540 	& 19229  \\
  \texttt{10BRIDGE} 		&	1565190 	 	& 54115 		&	?	 	& ?	 		& ?  		& ?	\\
  \texttt{DOORB} 		&	312579 	 	& 11782 		&	? 		& ?	 		& ?  		& ?	\\
  \texttt{LAVAB}  		&	177174 	 	& 3242 		&	? 		& 41860	 	& ?  		& ?	\\
  \texttt{TUNNEL}  		&	196810 	 	& 1879	 	&	? 		& 49		 	& ?  		& ?	\\
  \texttt{BREAD}  		&	26502 	 	& 8501 		&	? 		& ?	 		& 1124  	& 232\\
\end{tabular} 
}
\label{results_1}
\caption{Tests on a variety of tasks that require block placement and destruction actions}
\end{table}
...
results of the affordance aware subgoal planner. In the case of the bread baking world, standard value-iteration
...
In these cases, the affordance awareness did not improve any of the planners from their baseline versions.
These are precisely the results we would expect to see, as affordances are only beneficial in those cases
where powerful actions may be pruned. In each of these worlds, the agent only needed basic ``weak" actions,
such as opening doors, jumping, and movement. Thus, they are not good candidate tasks for affordance
aware planning.



% brief note about second table of results.

% somewhere explain what worlds are





Affordance aware planers, though effective in many cases, do fall short in some regards. For one,
they are specific to task types, as the lifted goal description $g$ of each affordance must be defined
with a task type (e.g. a predicate that defines the goal) in mind. Additionally, affordances (if defined optimally) 
...
in \texttt{BREADWORLD}, when the agent is up against a trench, it will still explore the space in which it places
...
current surroundings, and output a Multinouli distribution over the agent's action set. This would allow an affordance
aware planner to prioritize certain actions in each state, which would lead to pruning much larger portions of the state-space.
...
One of the most compelling results is the scope of task that affordance-aware planners are capable of solving. With an affordance-aware Subgoal planner (i.e. using an affordance aware RTDP as the low level planner), a Minecraft agent was able to solve a task in which it must open a door to find a block of grain, pick up the grain, avoid lava, build a bridge over a trench, and finally place the grain in the oven and bake it.
...
improve the effectiveness of the pruning. Currently, affordance-aware
planners remove actions that are inferred to be useless - this could
be extended to rank which actions are most useful in each state, thus
creating an opportunity to prune out actions that are considered less useful.
Further, we foresee extensions in natural language processing and information
extraction, in which affordances may be inferred via text or dialogue with a human partner.
...
receive affordance or subgoal {\it hints} from a human companion.
